...
export default function ProductPage() {
  ...
  return (
    <>
      <BreadCrumb title={product.name ? product.name : ""} />
      ...
      <div className="container-fluid my-3">
          ...
          <div className="col-md-6">
            ...
            <div className="table-responsive">
              <div className='my-2 d-flex justify-content-between'>
                {
                  product.pic.map((item, index) => {
                    return <img key={index} src={`${process.env.REACT_APP_BACKEND_SERVER}${item}`} data-bs-target="#carouselExampleIndicators" data-bs-slide-to={index} height={100} className='me-1 w-100' />
                  })
                }
              </div>
            </div>
                      ...
                      product.stock ? <div className="row">
                        <div className="col-5 col-sm-4 col-md-3">
                          <div className="btn-group w-100">
                            <button className='btn btn-primary' onClick={() => qty > 1 ? setQty(qty - 1) : null}><i className='bi bi-dash'></i></button>
                            <h3 className='w-50 text-center'>{qty}</h3>
                            <button className='btn btn-primary' onClick={() => qty < product.stockQuantity ? setQty(qty + 1) : null}><i className='bi bi-plus'></i></button>
                          </div>
                        </div>
                        <div className="col-7 col-sm-8 col-md-9">
                          <div className="btn-group w-100">
                            <button className='btn btn-primary' onClick={addToCart}><i className='bi bi-cart'></i>Add to Cart</button>
                            <button className='btn btn-secondary' onClick={addToWishlist}><i className='bi bi-heart'></i>Add to Wishlist</button>
                          </div>
                        </div>
                      </div> :
                        <button className='btn btn-secondary w-100' onClick={addToWishlist}><i className='bi bi-heart'></i>Add to Wishlist</button>
                    }
                  </td>
                </tr>
                <tr>
        ...
        <ProductsSlider title="Related Products" data={relatedProducts} />
      </div>
    </>
  )
}